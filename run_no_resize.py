...
parser.add_argument('--sh', type=int, default=40, help='source image height')
parser.add_argument('--sw', type=int, default=50, help='source image weight')

parser.add_argument('--ts', type=int, default=299, help='target image size')
parser.add_argument('--th', type=int, default=1080, help='target image height')
parser.add_argument('--tw', type=int, default=1920, help='target image weight')
parser.add_argument('--x', type=int, default=50, help='vertical location (center)')
parser.add_argument('--y', type=int, default=50, help='vertical location (center)')
...
target_json = os.path.join(opt.target_root, target_json_name)
...
x_center = th // 2
y_center = tw // 2
y_start = (box[0] + box[1]) // 2
x_start = (box[2] + box[3]) // 2

# x_center = ts // 2
# y_center = ts // 2
# y_start = (box[0] + box[1]) // 12
# x_start = (box[2] + box[3]) // 6
...
mask_img = utils.get_mask(source_file)
...
imsave(os.path.join(opt.output_dir, 'source_img.png'), source_img.astype(np.uint8))
...
second_pass_img = np.array(Image.open(first_pass_img_file).convert('RGB').resize((input_img_np.shape[0], input_img_np.shape[1])))
x1, x2, y1, y2 = int((x_start-opt.ss*0.5)*512/299), int((x_start+opt.ss*0.5)*512/299), int((y_start-opt.ss*0.5)*512/299), int((y_start+opt.ss*0.5)*512/299)
second_pass_img[x1:x2, y1:y2] = input_img_np[x1:x2, y1:y2]

imsave(os.path.join(opt.output_dir, 'second_pass_box.png'), second_pass_img.astype(np.uint8))
