...
def cutout4seg(jsonPath, jpgDirPath, add_pic_path):
    ...
    for num, json1 in enumerate(jsonName):
        ...
        # img = cv2.imdecode(np.fromfile(jpgDirPath + json1.split(".")[0] + ".jpg", dtype=np.uint8), -1)
        path = jsonPath + json1
        j = 0
        ...
        with open(path, 'r') as f:
            ...
            # img = cv2.imdecode(np.fromfile(jpgDirPath + imagePath, dtype=np.uint8), -1)
...
def change(old_pic_path, old_label_path, add_pic_path, new_pic_path, new_label_path):
    ...
    for num, i in enumerate(old_pic_path_list):
        ...
        label_num = random.sample(range(1, 2), 1)[0]
        # print("labelnum is ", label_num)
        if len(add_pic_path_list)  < label_num * 1:
            add_pic_path_list = os.listdir(add_pic_path)
        logging.info(num)
        # print(len(add_pic_path_list))
        print(num)
        old_pic_all_path = os.path.join(old_pic_path, i)
        # 每张图贴n次，这样至少能保证label铁道铁轨上
        for j in range(1):
            # 读原始图片
            # img_old = cv2.imdecode(np.fromfile(old_pic_all_path, dtype=np.uint8), -1)
            img_old = cv2.imread(old_pic_all_path)
            # 背景增强
            # if j > 0:
            #     # 利用HSV转换颜色和透明度
            #     img_hsv = cv2.cvtColor(img_old, cv2.COLOR_BGR2HSV)
            #     turn_green_hsv = img_hsv.copy()
            #     # # # 色调hue变化范围5——50
            #     # for i_h in range(1, 11):
            #     # 随机转换颜色
            #     # h_num = random.sample(range(100, 360), 1)[0]
            #     # turn_green_hsv[:, :, 0] = (turn_green_hsv[:, :, 0] + h_num) % 180
            #     # 随机调整透明度
            #     v_num = random.sample((0.7, 0.9, 1.1, 1.3), 1)[0]
            #     turn_green_hsv[:, :, 2] = v_num * turn_green_hsv[:, :, 2]
            #     turn_green_img = cv2.cvtColor(turn_green_hsv, cv2.COLOR_HSV2BGR)
            #     img_old = turn_green_img
            # 变暗
            # if j == 1:
            #     img_old = imgBrightness(img_old, 0.5, 0)
            # # 变亮
            # elif j == 2:
            #     img_old = imgBrightness(img_old, 1.5, 0)
            # 为原始jpg添加第四通道
            ...
            img_old = img_alpha
            # 随机选n个石头进行贴图
            add_stone_num = random.sample(range(0, len(add_pic_path_list)), label_num)
            add_stone_num.sort(reverse=True)
            # add_stone_num = random.sample(range(0, len(add_pic_path_list)), 1)
            with open("{}/{}_{}_{}_cover.txt".format(new_label_path, i.split(".")[0], num, j), "w") as f_write:
                ...
                for z in add_stone_num:
                    # add_pic_all_path = os.path.join(add_pic_path, add_i)
                    ...
                    # if j > 0:
                    #     v_num = random.sample((0.7, 0.9, 1.1, 1.3), 1)[0]
                    #     img_add = change_color(img_add, v_num)
                    # r_num = random.sample((0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330, 360), 1)[0]
                    r_num = random.sample(range(0, 3), 1)[0]
                    ###随机变明变暗
                    # if r_num == 0:
                    #     img_add = imgBrightness(img_add, 0.5, 0)
                    # elif r_num == 1:
                    #     img_add = imgBrightness(img_add, 1.5, 0)

                    # # # 随机旋转
                    # if r_num == 0:
                    #     img_add = cv2.flip(img_add, 1, dst=None)  # 水平镜像
                    # elif r_num == 1:
                    #     img_add = cv2.flip(img_add, 0, dst=None)  # 垂直镜像
                    # img_add = Rotate(img_add, r_num)
                    # cv2.imwrite("res11.jpg", img_add)
                    ...
                    for c in range(4):
                        img_alpha2[x1: x1 + add_w, y1: y1 + add_h, c] = img_add2 * img_alpha[x1: x1 + add_w,
                                                                                   y1: y1 + add_h,
                                                                                   c] + img_add1 * img_add[:, :, c]
                    ...
                    # cv2.imwrite("{}/{}_{}_{}_cover.jpg".format(new_pic_path, i.split(".")[0], num, j), img_alpha2)
                    # cv2.imencode('.jpg', img_alpha2)[1].tofile(filename)
...
if __name__ == '__main__':
    ...
    jsonPath = "./old_json_label/"  # 分割数据（石头）的labelme标注信息
    # jpgDirPath = "./json_pic/"  # 分割数据（石头）的labelme标注图片

    # # 从分割数据json文件中抠图，结果存入 add_pic
    # cutout4seg(jsonPath, jpgDirPath, add_pic_path)
    # # 把扣的图变为透明化
    # t = Transcolor()
    # photo_dir = 'add_pic'
