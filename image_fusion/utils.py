...
def get_mask(path, sh, sw):
    L_img = np.array(Image.open(path).convert('L').resize((sw, sh)))
    img = np.array(Image.open(path).resize((sw, sh)))
    w = img.shape[0]
    h = img.shape[1]
    for x in range(w):
        for y in range(h):
...
def make_canvas_mask(x_start, y_start, x_end, y_end, target_img, mask):
    canvas_mask = np.zeros((target_img.size[1], target_img.size[0]))
...
def compute_gt_gradient(x_start, y_start, x_end, y_end, source_img, target_img, mask, gpu_id):
